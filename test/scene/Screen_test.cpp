...
#define DELTA 1e-6
	...
	TEST_F(Screen_test, rotated90_zaxis_middlePixel) {
		Vector3D centerPixelLocation = testScreen_rotated.centerOfPixel(1, 1);
		ASSERT_NEAR(centerPixelLocation.getX(), origin_x + 1, DELTA);
		ASSERT_NEAR(centerPixelLocation.getY(), origin_y, DELTA);
	...
	TEST_F(Screen_test, rotated90_both_axes_middlePixel) {
		Vector3D topLeftPixel = testScreen_rotated_both_axis.centerOfPixel(1, 1);
		ASSERT_NEAR(topLeftPixel.getX(), origin_x, DELTA);
		ASSERT_NEAR(topLeftPixel.getY(), origin_y + 1, DELTA);
		ASSERT_NEAR(topLeftPixel.getZ(), origin_z, DELTA);
	}

	TEST_F(Screen_test, rotated90_both_axes_topLeftPixel) {
	...
	TEST_F(Screen_test, rotated90_both_axes_topRightPixel) {
	...
	TEST_F(Screen_test, rotated45_about_center_topLeftPixel) {
		...
		ASSERT_NEAR(topLeftPixel.getX(), origin_x, DELTA);
		ASSERT_NEAR(topLeftPixel.getY(), origin_y + sqrt(2) * 2/3, DELTA);
		ASSERT_NEAR(topLeftPixel.getZ(), origin_z + 1, DELTA);
	}
}